from .location.locationviews import location_views
from .locationrelation.locationrelationviews import location_relation_views
from .temperature.temperatureviews import temperature_views
from .precipitation.precipitationviews import precipitation_views

location_views = location_views()
location_relation_views = location_relation_views()
temperature_views = temperature_views()
precipitation_views = precipitation_views()
